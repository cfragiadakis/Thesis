...
class DataProcessor:
    @staticmethod
    def process_subject(subject_path, group):
        data = []
        ...
        image_data = DataProcessor._process_images(image_paths, labels, group, subject, experiment)
        sensor_data = DataProcessor._process_sensors(sensor_values, group, subject, experiment)
    ...
    @staticmethod
    def _process_images(image_paths, labels, group, subject, experiment):
        data = []
    ...
    @staticmethod
    def _process_sensors(sensor_values, group, subject, experiment):
        data = []
    ...
    def save_data_df(data, filename):
        ...
        if 'time' not in df.columns:
            logger.warning(f"[WARNING] Skipping {filename}, no time column")
            return
        ...
        df_sorted = df.sort_values(by=['group', 'experiment', 'subject', 'time'])

        for column in df_sorted.columns:
            if "emotion" in column or "attention" in column:
                df_sorted[column + "filled"] = df_sorted[column].ffill()
